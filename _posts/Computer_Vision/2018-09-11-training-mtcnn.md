...
* 2018.09.20
    ...
    整个系统连通后进行测试，发现人脸框抖动比较厉害。而且在Caffe上进行CPU运算时，速度极慢，尤其12net运行速度慢30倍左右。通过观察参数分布发现，有大量kernel都是全零分布，初步感觉是因为Adam和ignore label相互作用的结果，即ignore label的样本会产生0值loss，这些loss会影响Adam的优化过程。具体原因还需进一步理论推导。目前的解决方案是将含有大量0值kernel的层随机初始化，使用SGD进行训练。至于抖动的问题，需要进一步分析。
