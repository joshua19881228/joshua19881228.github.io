...
The key innovation of this work is an operation denoted as **ROI tracking**. The input of this operation is the bounding box regression features of the two frames $\textbf{x}^{t}_{reg}$, $\textbf{x}^{t+\tau}_{reg}$ and the correlation features $\textbf{x}^{t,t+\tau}_{corr}$, which are concatenated. The correlation layer performs point-wise feature comparison of two feature maps $\textbf{x}^{t}_{l}$, $\textbf{x}^{t+\tau}_{l}$
